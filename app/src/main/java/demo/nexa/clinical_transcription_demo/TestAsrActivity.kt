            ...
            try {
                ...
                for ((index, audioPath) in testAudioPaths.withIndex()) {
                    ...
                    for (run in 1..numRuns) {
                        ...
                        appendOutput("${inferenceTimeMs}ms, RTF=${String.format(Locale.US, "%.3f", rtf)}\n")
                    }
                    ...
                    appendOutput("  Avg Latency: ${String.format(Locale.US, "%.2f", avgLatency)}ms\n")
                    appendOutput("  Avg RTF: ${String.format(Locale.US, "%.3f", avgRtf)}\n")
                    appendOutput("  Min RTF: ${String.format(Locale.US, "%.3f", rtfs.minOrNull() ?: 0.0)}\n")
                    appendOutput("  Max RTF: ${String.format(Locale.US, "%.3f", rtfs.maxOrNull() ?: 0.0)}\n\n")
                ...
                if (allRtfs.isNotEmpty()) {
                    ...
                    appendOutput("Average RTF: ${String.format(Locale.US, "%.3f", allRtfs.average())}\n")
                    appendOutput("Min RTF: ${String.format(Locale.US, "%.3f", allRtfs.minOrNull() ?: 0.0)}\n")
                    appendOutput("Max RTF: ${String.format(Locale.US, "%.3f", allRtfs.maxOrNull() ?: 0.0)}\n\n")
                }
                    ...
                    transcribeResult.onSuccess { output ->
                        ...
                        if (timestamps != null && timestamps.isNotEmpty()) {
                            ...
                            if (intervals.isNotEmpty()) {
                                ...
                                appendOutput("Average timestamp interval: ${String.format(Locale.US, "%.1f", avgInterval)}ms\n")
                                appendOutput("Min interval: ${String.format(Locale.US, "%.1f", minInterval)}ms\n")
                                appendOutput("Max interval: ${String.format(Locale.US, "%.1f", maxInterval)}ms\n\n")
                                ...
                                for (i in 0 until minOf(10, timestamps.size)) {
                                    val intervalStr = if (i > 0) {
                                        " (${String.format(Locale.US, "%.1f", timestamps[i] - timestamps[i-1])}ms)"
                                    } else ""
                                    appendOutput("  [$i] ${String.format(Locale.US, "%.1f", timestamps[i])}ms$intervalStr\n")
                                }
                            }
                        } else {
                        ...
                        appendOutput("  TTFT: ${String.format(Locale.US, "%.2f", output.profileData.ttftMs)}ms\n")
                        appendOutput("  Prompt time: ${String.format(Locale.US, "%.2f", output.profileData.promptTimeMs)}ms\n")
                        appendOutput("  Decode time: ${String.format(Locale.US, "%.2f", output.profileData.decodeTimeMs)}ms\n")
                        appendOutput("  Audio duration: ${output.profileData.audioDurationMs}ms\n")
                        appendOutput("  SDK RTF: ${String.format(Locale.US, "%.3f", output.profileData.realTimeFactor)}\n")
                        appendOutput("  Prefill speed: ${String.format(Locale.US, "%.2f", output.profileData.prefillSpeed)} tokens/s\n")
                        appendOutput("  Decoding speed: ${String.format(Locale.US, "%.2f", output.profileData.decodingSpeed)} tokens/s\n")
            ...
            try {
                ...
                if (asrWrapper != null) {
                    ...
                    transcribeResult.onSuccess { output ->
                        ...
                        appendOutput("    ttftMs: ${String.format(Locale.US, "%.2f", output.profileData.ttftMs)}\n")
                        appendOutput("    promptTimeMs: ${String.format(Locale.US, "%.2f", output.profileData.promptTimeMs)}\n")
                        appendOutput("    decodeTimeMs: ${String.format(Locale.US, "%.2f", output.profileData.decodeTimeMs)}\n")
                        appendOutput("    audioDurationMs: ${output.profileData.audioDurationMs}\n")
                        appendOutput("    realTimeFactor: ${String.format(Locale.US, "%.3f", output.profileData.realTimeFactor)}\n")
                        appendOutput("    prefillSpeed: ${String.format(Locale.US, "%.2f", output.profileData.prefillSpeed)} tokens/s\n")
                        appendOutput("    decodingSpeed: ${String.format(Locale.US, "%.2f", output.profileData.decodingSpeed)} tokens/s\n")
