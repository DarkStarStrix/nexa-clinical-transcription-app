...
private data class ModelLoadConfig(
    ...
    val deviceIdCandidates: List<String?>,
    val defaultBackendLabel: String
)
...
class NexaLlmEngine private constructor(
    ...
    @Volatile
    private var lastBackendLabel: String = "unknown"
    ...
    suspend fun loadModel(modelType: LlmModelManager.ModelType) = withContext(Dispatchers.IO) {
        modelMutex.withLock {
            ...
            val modelConfig = when (modelType) {
                LlmModelManager.ModelType.LIQUID_SUMMARIZER -> {
                    ...
                    ModelLoadConfig(
                        ...
                        pluginId = plugin,
                        config = cfg,
                        deviceIdCandidates = listOf("HTP0", "GPUOpenCL", null),
                        defaultBackendLabel = "auto"
                    )
                }
            ...
            for (candidate in modelConfig.deviceIdCandidates) {
                ...
                result.onSuccess { wrapper ->
                    ...
                    lastBackendLabel = candidate ?: "CPU"
                    loaded = true
                }.onFailure { error ->
                    lastError = error
                }
            ...
            if (currentWrapper == null || currentModelType != modelType) {
                val err = lastError ?: IllegalStateException("Unknown error loading ${modelType.name}")
                Log.e(TAG, "Failed to load ${modelType.name} model (all backends)", err)
                throw Exception("Failed to load ${modelType.name}: ${err.message}", err)
            }
        }
    }
    ...
    fun generateSoapSummary(transcript: String): Flow<SoapGenerationResult> = channelFlow {
        withContext(Dispatchers.IO) {
            ...
            // Prefer Qwen for final SOAP creation when available, otherwise fall back to Liquid.
            val creatorModel = try {
                loadModel(LlmModelManager.ModelType.QWEN_SOAP_CREATOR)
                LlmModelManager.ModelType.QWEN_SOAP_CREATOR
            } catch (e: Exception) {
                Log.w(TAG, "Qwen model unavailable; falling back to Liquid for SOAP creation", e)
                loadModel(LlmModelManager.ModelType.LIQUID_SUMMARIZER)
                LlmModelManager.ModelType.LIQUID_SUMMARIZER
            }

            val wrapper = currentWrapper ?: throw IllegalStateException("Failed to load creator model ($creatorModel)")
    ...
    fun getLastBackendLabel(): String = lastBackendLabel
